...
function MarketplaceConfigModal({market,form,setForm,onClose}){
  const [requiredAttrs,setRequiredAttrs]=useState([]),[attrsBusy,setAttrsBusy]=useState(false),[storesInfo,setStoresInfo]=useState({warehouse_management:false,multiwarehouse:false,stores:[]})
...
function ProductForm({refresh,notice,groups=[],brands=[],vehicles=[],locations=[],initialProduct=null,onClose=null}){
  ...
  async function searchCategory(){const q=String(categoryQuery||form.name||'').trim();if(!q)return notice('Digite o nome da peça ou da categoria');setCategoryBusy(true);try{const r=await api.get('/marketplaces/mercadolivre/category-suggestions',{params:{q,limit:8}});setCategoryResults(r.data||[]);if(!(r.data||[]).length)notice('Nenhuma categoria encontrada')}catch(e){notice(erroPt(e.response?.data?.detail)||'Não foi possível pesquisar as categorias do Mercado Livre')}finally{setCategoryBusy(false)}}
  function chooseCategory(item){setForm({...form,category:item.category_name||'',ml_category_id:item.category_id||''});setCategoryOpen(false);setCategoryQuery('')}
  ...
  const channel=(id,label,field,desc)=><div className={'channelToggle '+(form[field]?'enabled':'')}><div><MarketLogo id={id}/><span><b>{label}</b><small>{desc}</small></span></div><label className="switch"><input type="checkbox" checked={!!form[field]} onChange={e=>setForm({...form,[field]:e.target.checked})}/><i/></label><button type="button" className="ghost" disabled={!form[field]} onClick={()=>setMarketModal(id)}>Configurar</button></div>
  ...
  return <section className={editing?'productEditorBody':'panel productRegisterPage'}>
    <PanelHead eyebrow={editing?'EDIÇÃO':'CATÁLOGO'} title={editing?`Editar peça #${initialProduct.id}`:'Cadastrar nova peça'} text={editing?'Altere os dados e salve no mesmo produto.':'Cadastro rápido, organizado e pronto para estoque e canais de venda.'}/>
      ...
      <div className="productRegisterMain">
        {step==='cadastro'&&<>
          <div className="productSectionCard"><div className="productSectionHead"><div><span>PASSO 1</span><h3>Cadastro da peça</h3><p>Preencha primeiro os dados que você usa no dia a dia.</p></div></div><div className="productSectionBody"><div className="productCoreGrid">
            ...
            <Field label="Ano"><input list="cdm-product-years" inputMode="numeric" maxLength="4" value={form.year||''} onChange={e=>setForm({...form,year:e.target.value.replace(/\D/g,'').slice(0,4)})} placeholder="Ex.: 2020"/><datalist id="cdm-product-years">{Array.from({length:new Date().getFullYear()-1969},(_,i)=>new Date().getFullYear()+1-i).map(y=><option key={y} value={y}/>)}</datalist></Field>
            <Field label="Categoria"><div className="categoryField"><input value={form.category||''} onChange={e=>setForm({...form,category:e.target.value})} placeholder="Digite ou pesquise uma categoria"/><button type="button" className="ghost categorySearchBtn" onClick={()=>{setCategoryQuery(form.name||form.category||'');setCategoryResults([]);setCategoryOpen(true)}}>Pesquisar</button></div></Field>
          ...
          <details className="productAdvanced"><summary>Mais informações da peça</summary><div className="productCoreGrid advancedGrid">
            <Field label="Lado"><select value={form.side||''} onChange={e=>setForm({...form,side:e.target.value})}><option value="">Não se aplica</option><option>Direito</option><option>Esquerdo</option></select></Field>
            <Field label="Posição"><select value={form.position||''} onChange={e=>setForm({...form,position:e.target.value})}><option value="">Não se aplica</option><option>Dianteira</option><option>Traseira</option><option>Superior</option><option>Inferior</option></select></Field>
            ...
            <div className="span2"><Field label="Descrição"><textarea value={form.description||''} onChange={e=>setForm({...form,description:e.target.value})} placeholder="Detalhes da peça..."/></Field></div>
            <div className="span2"><Field label="Compatibilidade"><textarea value={form.compatibility||''} onChange={e=>setForm({...form,compatibility:e.target.value})} placeholder="Ex.: Corolla 2015 a 2019"/></Field></div>
            <div className="span4"><Field label="Observações de qualidade"><textarea value={form.quality_notes||''} onChange={e=>setForm({...form,quality_notes:e.target.value})} placeholder="Riscos, marcas, testes realizados..."/></Field></div>
            <label className="inlineCheck span4"><input type="checkbox" checked={form.public_catalog!==false} onChange={e=>setForm({...form,public_catalog:e.target.checked})}/><span>Permitir esta peça no catálogo público da empresa</span></label>
          </div></details></div></div>
        </>}

        {step==='midia'&&<div className="productSectionCard"><div className="productSectionHead"><div><span>PASSO 2</span><h3>Mídia da peça</h3><p>Adicione fotos e deixe o produto pronto para anúncio.</p></div></div><div className="productSectionBody"><ProductImages form={form} setForm={setForm} notice={notice}/></div></div>}

        {step==='publicacao'&&<><div className="productSectionCard"><div className="productSectionHead"><div><span>PASSO 3</span><h3>Publicação</h3><p>Escolha os canais. Você pode apenas salvar a peça sem publicar.</p></div></div><div className="productSectionBody"><div className="channelToggles">{channel('mercadolivre','Mercado Livre','publish_mercadolivre','Ativar ou desativar esta peça no Mercado Livre.')}{channel('shopee','Shopee','publish_shopee','Ativar ou desativar esta peça na Shopee.')}{channel('olx','OLX','publish_olx','Ativar ou desativar esta peça na OLX.')}</div>{form.publish_mercadolivre&&<div className="mlQuick"><button type="button" className="ghost" onClick={suggestMl} disabled={findingCategory}>{findingCategory?'Buscando...':'Sugerir categoria do Mercado Livre'}</button>{mlSuggestions.length>0&&<select value={form.ml_category_id||''} onChange={e=>setForm({...form,ml_category_id:e.target.value})}>{mlSuggestions.map(x=><option key={x.category_id} value={x.category_id}>{x.category_name} · {x.category_id}</option>)}</select>}<input placeholder="ID da categoria MLB..." value={form.ml_category_id||''} onChange={e=>setForm({...form,ml_category_id:e.target.value})}/></div>}</div></div>
          <div className="productSectionCard"><div className="productSectionHead compact"><div><h3>Peso e embalagem</h3><p>Necessário para cálculo de frete e publicação.</p></div></div><div className="productSectionBody"><div className="dimensions"><Field label="Peso (kg)"><input type="number" step="0.01" value={form.weight??''} onChange={e=>setForm({...form,weight:e.target.value})}/></Field><Field label="Comprimento (cm)"><input type="number" value={form.package_length??''} onChange={e=>setForm({...form,package_length:e.target.value})}/></Field><Field label="Largura (cm)"><input type="number" value={form.package_width??''} onChange={e=>setForm({...form,package_width:e.target.value})}/></Field><Field label="Altura (cm)"><input type="number" value={form.package_height??''} onChange={e=>setForm({...form,package_height:e.target.value})}/></Field></div></div></div></>}

        {step==='precificacao'&&<div className="productSectionCard"><div className="productSectionHead"><div><span>PASSO 4</span><h3>Precificação</h3><p>Informe custo e preço sem zeros preenchidos automaticamente.</p></div></div><div className="productSectionBody"><div className="pricingGrid"><Field label="Custo de compra"><input type="number" step="0.01" value={form.cost??''} onChange={e=>setForm({...form,cost:e.target.value})} placeholder="R$ 0,00"/></Field><Field label="Preço de venda"><input type="number" step="0.01" value={form.price??''} onChange={e=>setForm({...form,price:e.target.value})} placeholder="R$ 0,00"/></Field><div className="pricingMetric"><small>Lucro estimado</small><strong>{money(profit)}</strong><span>{price>0?`${margin.toFixed(1)}% de margem`:'Informe o preço de venda'}</span></div></div><div className="markupButtons"><span>Aplicar margem sobre o custo:</span>{[30,50,80,100].map(v=><button type="button" className="ghost" key={v} onClick={()=>applyMarkup(v)}>+{v}%</button>)}</div></div></div>}
      </div>

      <aside className="productRegisterSide">
        <div className="productStepNav">{steps.map(([id,label],idx)=><button type="button" key={id} className={step===id?'active':''} onClick={()=>setStep(id)}><span>{idx+1}</span>{label}</button>)}</div>
        <div className="productQuickSummary"><small>RESUMO</small><b>{form.name||'Nova peça'}</b><span>{form.sku||'SKU não informado'}</span><div><em>Qtd.</em><strong>{form.stock||0}</strong></div><div><em>Preço</em><strong>{form.price?money(form.price):'—'}</strong></div></div>
        <div className="productSideActions">{editing&&<button className="ghost" onClick={onClose}>Cancelar</button>}<button className="ghost strong" onClick={()=>save(false)}>{editing?'Salvar alterações':'Salvar peça'}</button><button className="primary" onClick={()=>save(true)}>{editing?'Salvar e sincronizar':'Salvar e publicar'}</button></div>
      </aside>
    </div>
    ...
    <MarketplaceConfigModal market={marketModal} form={form} setForm={setForm} onClose={()=>setMarketModal(null)}/>
  </section>
}
...
function LocationsModule({rows,refresh,notice}){
  ...
  const [open,setOpen]=useState(false),[form,setForm]=useState(empty)
  const path=[form.warehouse,form.aisle,form.shelf,form.bin].filter(Boolean).join(' › ')
  async function save(){
    try{
      if(!form.description.trim())return notice('Informe o nome da localização')
      const payload={...form,auto_generate:true,code:'',max_quantity:Number(form.max_quantity||0)}
      await api.post('/catalog/locations',payload)
      setForm(empty);setOpen(false);await refresh();notice('Localização cadastrada')
    }catch(e){notice(erroPt(e.response?.data?.detail)||'Erro ao salvar localização')}
  }
  return <>
    <div className="pageTitle"><div><span>CADASTROS</span><h2>Localizações do estoque</h2><p>Crie locais fáceis de entender, como Depósito 1 › Corredor A › Prateleira 2 › Posição 3.</p></div><button className="primary" onClick={()=>{setForm(empty);setOpen(true)}}>＋ Nova localização</button></div>
    <section className="panel"><div className="locationList">{rows.length?rows.map(l=><div className="locationRow" key={l.id}><div className="locationCode">{l.code||`LOC-${l.id}`}</div><div><b>{l.description||'Localização'}</b><small>{[l.warehouse,l.aisle,l.shelf,l.bin].filter(Boolean).join(' › ')||'Sem detalhes adicionais'}</small></div><span>{l.max_quantity?`Limite: ${l.max_quantity}`:'Sem limite'}</span><span className={l.active!==false?'pill success':'pill neutral'}>{l.active!==false?'Ativa':'Inativa'}</span></div>):<div className="emptyState">Nenhuma localização cadastrada. Clique em “Nova localização” para começar.</div>}</div></section>
    {open&&<div className="modalBackdrop" onMouseDown={e=>e.target===e.currentTarget&&setOpen(false)}><div className="v8Modal locationEasyModal">
      <div className="modalHead"><div><small>ESTOQUE</small><h2>Nova localização</h2><p>Você só precisa dar um nome. O código é criado automaticamente pelo sistema.</p></div><button className="iconClose" onClick={()=>setOpen(false)}>×</button></div>
      <div className="modalBody">
        <div className="locationHelp"><b>Exemplo simples</b><span>Nome: Prateleira de faróis</span><span>Depósito: Principal · Corredor: A · Prateleira: 2 · Posição: 3</span></div>
        <div className="locationSimpleGrid">
          <div className="locationNameField"><Field label="Nome da localização *"><input autoFocus className={!form.description?'requiredInput':''} placeholder="Ex.: Prateleira de faróis" value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/></Field><small>Use um nome que qualquer pessoa da equipe entenda.</small></div>
          <Field label="Depósito / Área"><input placeholder="Ex.: Principal" value={form.warehouse} onChange={e=>setForm({...form,warehouse:e.target.value})}/></Field>
          <Field label="Corredor"><input placeholder="Ex.: A" value={form.aisle} onChange={e=>setForm({...form,aisle:e.target.value.toUpperCase()})}/></Field>
          <Field label="Prateleira"><input placeholder="Ex.: 2" value={form.shelf} onChange={e=>setForm({...form,shelf:e.target.value})}/></Field>
          <Field label="Posição"><input placeholder="Ex.: 3" value={form.bin} onChange={e=>setForm({...form,bin:e.target.value})}/></Field>
        </div>
        <div className="locationPreview"><small>COMO VAI APARECER</small><b>{form.description||'Nome da localização'}</b><span>{path||'Preencha depósito, corredor, prateleira ou posição se quiser detalhar.'}</span><em>O código será gerado automaticamente ao salvar.</em></div>
        <details className="locationAdvanced"><summary>Opções avançadas</summary><div><Field label="Limite de peças (opcional)"><input type="number" min="0" value={form.max_quantity} onChange={e=>setForm({...form,max_quantity:e.target.value})} placeholder="Deixe vazio para sem limite"/></Field></div></details>
      </div>
      <div className="modalFoot"><button className="ghost" onClick={()=>setOpen(false)}>Cancelar</button><button className="primary" onClick={save}>Salvar localização</button></div>
    </div></div>}
  </>
}

function SuppliersModule({rows,refresh,notice}){const empty={name:'',trade_name:'',cpf_cnpj:'',rg_ie:'',mobile:'',phone:'',cep:'',city:'',state:'RJ',number:'',address:'',neighborhood:'',complement:'',ibge:'',email:'',active:true},[open,setOpen]=useState(false),[form,setForm]=useState(empty),[cepBusy,setCepBusy]=useState(false);async function buscarCep(){const cep=form.cep.replace(/\D/g,'');if(cep.length!==8)return;setCepBusy(true);try{const r=await fetch(`https://viacep.com.br/ws/${cep}/json/`),d=await r.json();if(!d.erro)setForm(f=>({...f,address:d.logradouro||f.address,neighborhood:d.bairro||f.neighborhood,city:d.localidade||f.city,state:d.uf||f.state,ibge:d.ibge||f.ibge}))}catch(e){}finally{setCepBusy(false)}}async function save(){try{if(!form.cpf_cnpj.trim()||!form.name.trim())return notice('Preencha CPF / CNPJ e Razão Social / Nome');await api.post('/catalog/suppliers',form);setForm(empty);setOpen(false);await refresh();notice('Fornecedor cadastrado')}catch(e){notice(erroPt(e.response?.data?.detail)||'Erro ao salvar fornecedor')}}return <><div className="pageTitle"><div><span>CADASTROS</span><h2>Fornecedores</h2><p>Dados comerciais, fiscais, contato e endereço em um único cadastro.</p></div><button className="primary" onClick={()=>setOpen(true)}>＋ Adicionar fornecedor</button></div><section className="panel"><Table rows={rows} cols={['id','cpf_cnpj','name','trade_name','mobile','city','state','email','active']} format={{active:v=>v!==false?'Ativo':'Inativo'}}/></section>{open&&<div className="modalBackdrop" onMouseDown={e=>e.target===e.currentTarget&&setOpen(false)}><div className="v8Modal large"><div className="modalHead"><div><small>FORNECEDOR</small><h2>Cadastrar fornecedor</h2><p>Os campos marcados com * são obrigatórios.</p></div><button className="iconClose" onClick={()=>setOpen(false)}>×</button></div><div className="modalBody"><div className="formGrid three"><Field label="CPF / CNPJ *"><input className={!form.cpf_cnpj?'requiredInput':''} value={form.cpf_cnpj} onChange={e=>setForm({...form,cpf_cnpj:e.target.value})}/></Field><Field label="Razão Social / Nome *"><input className={!form.name?'requiredInput':''} value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></Field><Field label="Nome Fantasia"><input value={form.trade_name} onChange={e=>setForm({...form,trade_name:e.target.value})}/></Field><Field label="RG / Inscrição Estadual"><input value={form.rg_ie} onChange={e=>setForm({...form,rg_ie:e.target.value})}/></Field><Field label="Celular"><input value={form.mobile} onChange={e=>setForm({...form,mobile:e.target.value})}/></Field><Field label="Telefone"><input value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})}/></Field><Field label="CEP"><div className="inputAction"><input value={form.cep} onBlur={buscarCep} onChange={e=>setForm({...form,cep:e.target.value})}/><button className="ghost" onClick={buscarCep}>{cepBusy?'...':'Buscar'}</button></div></Field><Field label="Cidade"><input value={form.city} onChange={e=>setForm({...form,city:e.target.value})}/></Field><Field label="UF"><input maxLength="2" value={form.state} onChange={e=>setForm({...form,state:e.target.value.toUpperCase()})}/></Field><Field label="Número"><input value={form.number} onChange={e=>setForm({...form,number:e.target.value})}/></Field><Field label="Logradouro"><input value={form.address} onChange={e=>setForm({...form,address:e.target.value})}/></Field><Field label="Bairro"><input value={form.neighborhood} onChange={e=>setForm({...form,neighborhood:e.target.value})}/></Field><Field label="Complemento"><input value={form.complement} onChange={e=>setForm({...form,complement:e.target.value})}/></Field><Field label="Código IBGE"><input value={form.ibge} onChange={e=>setForm({...form,ibge:e.target.value})}/></Field><Field label="E-mail"><input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></Field></div><label className="inlineCheck"><input type="checkbox" checked={form.active} onChange={e=>setForm({...form,active:e.target.checked})}/><span>Fornecedor ativo</span></label></div><div className="modalFoot"><button className="ghost" onClick={()=>setOpen(false)}>Cancelar</button><button className="primary" onClick={save}>Salvar fornecedor</button></div></div></div>}</>}
...
function FiscalModule({tab,notice}){
 const fiscalEmpty={
  series:'1',number:'',operation_type:'saida',purpose:'normal',operation_nature:'Venda de mercadoria',referenced_key:'',order_number:'',intermediary_indicator:'sem_intermediador',recipient:'',recipient_ie:'',
  sections:{
   items:{description:'',quantity:1,unit:'UN',unit_value:0,ncm:'',cfop:'5102'},
   transport:{freight_mode:'sem_frete',carrier:'',plate:'',state:'RJ',volumes:0,gross_weight:0,net_weight:0},
   financial:{payment_method:'pix',amount:0,installments:1,due_date:''},
   taxation:{regime:'Simples Nacional',cfop:'5102',ncm:'',csosn:'102',icms_rate:0,pis_rate:0,cofins_rate:0,ipi_rate:0}
  }
 }
 const [form,setForm]=useState(fiscalEmpty),[section,setSection]=useState('identification'),[docs,setDocs]=useState([]),[docId,setDocId]=useState(null),[busy,setBusy]=useState(false)
 const setSec=(key,field,value)=>setForm(f=>({...f,sections:{...f.sections,[key]:{...(f.sections?.[key]||{}),[field]:value}}}))
 async function load(){try{setDocs((await api.get('/fiscal')).data||[])}catch(e){}}
 useEffect(()=>{load()},[tab])
 ...
 async function emit(){let id=docId;if(!id)id=await saveDraft();if(!id)return;setBusy(true);try{await api.post(`/fiscal/${id}/emit`);await load();notice('NF-e enviada para emissão')}catch(e){notice(erroPt(e.response?.data?.detail)||'Não foi possível emitir a NF-e')}finally{setBusy(false)}}
 function newDraft(){setForm(fiscalEmpty);setDocId(null);setSection('identification')}
 if(tab==='invoice-history')return <section className="panel"><PanelHead eyebrow="FISCAL" title="Notas emitidas e rascunhos" text="Histórico dos documentos fiscais da empresa."/><Table rows={docs} cols={['id','number','series','recipient','status','access_key','created_at']} format={{status:v=>statusPt(v),created_at:v=>v?new Date(v).toLocaleString('pt-BR'):'—'}}/></section>
 if(tab==='xml')return <SimpleModule eyebrow="FISCAL" title="Enviar XML" text="Importe e organize arquivos XML fiscais." actions={['Selecionar XML','Consultar documentos','Configuração fiscal']}/>
 if(tab==='invalidate-number')return <SimpleModule eyebrow="FISCAL" title="Inutilizar número" text="Área preparada para inutilização de numeração fiscal pelo provedor configurado." actions={['Informar faixa','Justificativa','Consultar histórico']}/>
 const tabs=[['identification','Identificação'],['items','Itens'],['transport','Transporte'],['financial','Financeiro'],['taxation','Tributação']]
 const items=form.sections?.items||{},transport=form.sections?.transport||{},financial=form.sections?.financial||{},taxation=form.sections?.taxation||{}
 return <><div className="pageTitle fiscalTitle"><div><span>FISCAL</span><h2>Emitir NF-e - AMBIENTE DE PRODUÇÃO</h2><p>Identificação, itens, transporte, financeiro e tributação em uma única emissão.</p></div><span className="fiscalEnv">PRODUÇÃO</span></div><section className="panel fiscalPanel"><div className="fiscalTabs">{tabs.map(([k,l])=><button className={section===k?'active':''} key={k} onClick={()=>setSection(k)}>{l}</button>)}</div>
 {section==='identification'&&<div className="formGrid three fiscalForm"><Field label="Série"><input value={form.series} onChange={e=>setForm({...form,series:e.target.value})}/></Field><Field label="Número"><input value={form.number} onChange={e=>setForm({...form,number:e.target.value})}/></Field><Field label="Tipo de operação"><select value={form.operation_type} onChange={e=>setForm({...form,operation_type:e.target.value})}><option value="saida">Saída</option><option value="entrada">Entrada</option></select></Field><Field label="Finalidade da NF-e"><select value={form.purpose} onChange={e=>setForm({...form,purpose:e.target.value})}><option value="normal">Normal</option><option value="complementar">Complementar</option><option value="ajuste">Ajuste</option><option value="devolucao">Devolução</option></select></Field><Field label="Natureza da operação"><input value={form.operation_nature} onChange={e=>setForm({...form,operation_nature:e.target.value})}/></Field><Field label="Chave da NF-e referenciada"><input value={form.referenced_key} onChange={e=>setForm({...form,referenced_key:e.target.value})}/></Field><Field label="Número do pedido"><input value={form.order_number} onChange={e=>setForm({...form,order_number:e.target.value})}/></Field><Field label="Indicador de intermediador"><select value={form.intermediary_indicator} onChange={e=>setForm({...form,intermediary_indicator:e.target.value})}><option value="sem_intermediador">Sem intermediador</option><option value="com_intermediador">Com intermediador</option></select></Field><Field label="Destinatário"><input value={form.recipient} onChange={e=>setForm({...form,recipient:e.target.value})}/></Field><Field label="IE do destinatário"><input value={form.recipient_ie} onChange={e=>setForm({...form,recipient_ie:e.target.value})}/></Field></div>}
 {section==='items'&&<div className="fiscalSectionEditor"><div className="sectionIntro"><h3>Itens da NF-e</h3><p>Informe os dados da peça ou mercadoria desta nota.</p></div><div className="formGrid three"><Field label="Descrição da peça"><input value={items.description||''} onChange={e=>setSec('items','description',e.target.value)}/></Field><Field label="Quantidade"><input type="number" min="1" value={items.quantity??1} onChange={e=>setSec('items','quantity',+e.target.value)}/></Field><Field label="Unidade"><input value={items.unit||'UN'} onChange={e=>setSec('items','unit',e.target.value.toUpperCase())}/></Field><Field label="Valor unitário"><input type="number" step="0.01" value={items.unit_value??0} onChange={e=>setSec('items','unit_value',+e.target.value)}/></Field><Field label="NCM"><input value={items.ncm||''} onChange={e=>setSec('items','ncm',e.target.value)}/></Field><Field label="CFOP"><input value={items.cfop||'5102'} onChange={e=>setSec('items','cfop',e.target.value)}/></Field></div><div className="fiscalTotalPreview"><span>Total do item</span><b>{money(Number(items.quantity||0)*Number(items.unit_value||0))}</b></div></div>}
 {section==='transport'&&<div className="fiscalSectionEditor"><div className="sectionIntro"><h3>Transporte</h3><p>Defina frete, transportadora e volumes.</p></div><div className="formGrid three"><Field label="Modalidade do frete"><select value={transport.freight_mode||'sem_frete'} onChange={e=>setSec('transport','freight_mode',e.target.value)}><option value="sem_frete">Sem frete</option><option value="emitente">Por conta do emitente</option><option value="destinatario">Por conta do destinatário</option><option value="terceiros">Por conta de terceiros</option></select></Field><Field label="Transportadora"><input value={transport.carrier||''} onChange={e=>setSec('transport','carrier',e.target.value)}/></Field><Field label="Placa"><input value={transport.plate||''} onChange={e=>setSec('transport','plate',e.target.value.toUpperCase())}/></Field><Field label="UF do veículo"><input maxLength="2" value={transport.state||'RJ'} onChange={e=>setSec('transport','state',e.target.value.toUpperCase())}/></Field><Field label="Volumes"><input type="number" min="0" value={transport.volumes??0} onChange={e=>setSec('transport','volumes',+e.target.value)}/></Field><Field label="Peso bruto (kg)"><input type="number" step="0.01" min="0" value={transport.gross_weight??0} onChange={e=>setSec('transport','gross_weight',+e.target.value)}/></Field><Field label="Peso líquido (kg)"><input type="number" step="0.01" min="0" value={transport.net_weight??0} onChange={e=>setSec('transport','net_weight',+e.target.value)}/></Field></div></div>}
 {section==='financial'&&<div className="fiscalSectionEditor"><div className="sectionIntro"><h3>Financeiro</h3><p>Forma de pagamento e vencimento do documento.</p></div><div className="formGrid three"><Field label="Forma de pagamento"><select value={financial.payment_method||'pix'} onChange={e=>setSec('financial','payment_method',e.target.value)}><option value="pix">PIX</option><option value="dinheiro">Dinheiro</option><option value="cartao_credito">Cartão de crédito</option><option value="cartao_debito">Cartão de débito</option><option value="boleto">Boleto</option><option value="transferencia">Transferência</option></select></Field><Field label="Valor"><input type="number" step="0.01" min="0" value={financial.amount??0} onChange={e=>setSec('financial','amount',+e.target.value)}/></Field><Field label="Parcelas"><input type="number" min="1" value={financial.installments??1} onChange={e=>setSec('financial','installments',+e.target.value)}/></Field><Field label="Vencimento"><input type="date" value={financial.due_date||''} onChange={e=>setSec('financial','due_date',e.target.value)}/></Field></div></div>}
 {section==='taxation'&&<div className="fiscalSectionEditor"><div className="sectionIntro"><h3>Tributação</h3><p>Dados fiscais principais usados no cálculo e envio ao provedor.</p></div><div className="formGrid three"><Field label="Regime tributário"><select value={taxation.regime||'Simples Nacional'} onChange={e=>setSec('taxation','regime',e.target.value)}><option>Simples Nacional</option><option>Lucro Presumido</option><option>Lucro Real</option></select></Field><Field label="CFOP"><input value={taxation.cfop||'5102'} onChange={e=>setSec('taxation','cfop',e.target.value)}/></Field><Field label="NCM"><input value={taxation.ncm||''} onChange={e=>setSec('taxation','ncm',e.target.value)}/></Field><Field label="CSOSN / CST"><input value={taxation.csosn||'102'} onChange={e=>setSec('taxation','csosn',e.target.value)}/></Field><Field label="Alíquota ICMS (%)"><input type="number" step="0.01" value={taxation.icms_rate??0} onChange={e=>setSec('taxation','icms_rate',+e.target.value)}/></Field><Field label="Alíquota PIS (%)"><input type="number" step="0.01" value={taxation.pis_rate??0} onChange={e=>setSec('taxation','pis_rate',+e.target.value)}/></Field><Field label="Alíquota COFINS (%)"><input type="number" step="0.01" value={taxation.cofins_rate??0} onChange={e=>setSec('taxation','cofins_rate',+e.target.value)}/></Field><Field label="Alíquota IPI (%)"><input type="number" step="0.01" value={taxation.ipi_rate??0} onChange={e=>setSec('taxation','ipi_rate',+e.target.value)}/></Field></div></div>}
 <div className="fiscalActions"><button className="primary" disabled={busy} onClick={emit}>{busy?'Processando...':'Emitir NF-e'}</button><button className="ghost" disabled={busy} onClick={saveDraft}>Salvar rascunho</button><button className="ghost" onClick={newDraft}>Cancelar</button></div><div className="fiscalWarning">A emissão real depende de provedor fiscal, certificado digital e credenciais configuradas no servidor. O CDM não simula autorização da SEFAZ.</div></section></>
}
