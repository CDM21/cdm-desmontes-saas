...
function ProductForm({refresh,notice,groups=[],brands=[],vehicles=[],locations=[],initialProduct=null,onClose=null}){
  ...
  return <section className={editing?'productEditorBody':'panel productRegisterPage'}>
    ...
    <div className="productEditorPremiumHero"><div className="productEditorPremiumLead"><span>CDM • CADASTRO PROFISSIONAL</span><strong>{editing?'Editar peça':'Nova peça'}</strong><small>Cadastro organizado para estoque, precificação e marketplaces sem repetir informações.</small></div><div className="productEditorPremiumStats"><div><span>SKU</span><b>{form.sku||'—'}</b></div><div><span>Peça</span><b>{form.name||'Ainda não informada'}</b></div><div><span>Veículo</span><b>{[form.brand,form.model,form.year].filter(Boolean).join(' ')||'—'}</b></div><div><span>Preço</span><b>{form.price?money(form.price):'—'}</b></div></div></div>
