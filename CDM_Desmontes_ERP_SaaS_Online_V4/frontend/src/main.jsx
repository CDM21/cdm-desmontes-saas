...
function CompanyInfo({session,reload,notice}){
 ...
 async function lookup(){const c=String(form.cnpj||'').replace(/\D/g,'');if(c.length!==14)return notice('Digite um CNPJ válido com 14 números');setLookupBusy(true);try{const r=await api.get(`/fiscal/cnpj/${c}`);setForm(v=>({...v,...r.data,state_registration:v.state_registration,tax_regime:v.tax_regime,responsible_name:v.responsible_name,rg:v.rg,cpf:v.cpf,issuing_agency:v.issuing_agency}));notice('Dados públicos do CNPJ preenchidos. Confira e salve.')}catch(e){notice(erroPt(e.response?.data?.detail)||'Não foi possível consultar o CNPJ')}finally{setLookupBusy(false)}}
 ...
 return <>
   ...
   <section className="panel info integrationHelp">
     <div>
       ...
       <p><b>Conectado:</b> conta autorizada. <b>Pronto para conectar:</b> servidor configurado, falta o cliente autorizar. <b>Disponível em breve:</b> o canal ainda não foi liberado pela plataforma. <b>Conectado com alerta:</b> a conta existe, mas houve erro recente.</p>
     </div>
   ...
   {diagData&&<div className="modalBackdrop" onMouseDown={e=>e.target===e.currentTarget&&setDiagData(null)}>
     <div className="v8Modal medium integrationDiagModal">
       <div className="modalHead">
         <div><small>DIAGNÓSTICO DO CANAL</small><h2>{marketName(diagData.id)}</h2><p>Verificação automática da conexão sem expor dados sensíveis.</p></div>
...
function FiscalModule({tab,notice}){
 ...
 async function load(){try{const [d,c]=await Promise.all([api.get('/fiscal'),api.get('/fiscal/setup')]);setDocs(d.data||[]);setSetup(c.data)}catch(e){}}
 useEffect(()=>{load()},[tab])
 ...
 async function sendCert(){if(!cert)return notice('Selecione seu certificado A1 (.pfx ou .p12)');setSetupBusy(true);try{const fd=new FormData();fd.append('certificate',cert);fd.append('password',certPass);const r=await api.post('/fiscal/setup/certificate',fd,{headers:{'Content-Type':'multipart/form-data'}});setSetup(r.data);setCertPass('');notice('Certificado enviado e validado pelo provedor fiscal')}catch(e){notice(erroPt(e.response?.data?.detail)||'Não foi possível configurar o certificado')}finally{setSetupBusy(false)}}
 ...
 if(!setup.ready)return <><div className="pageTitle"><div><span>CONFIGURAÇÃO FISCAL GUIADA</span><h2>Prepare a emissão de NF-e</h2><p>Faça uma vez. Depois o CDM usa essa configuração automaticamente.</p></div><span className="fiscalEnv">{setup.environment==='producao'?'PRODUÇÃO':'HOMOLOGAÇÃO'}</span></div><section className="panel fiscalSetupWizard"><div className="setupHero"><div><b>Você não precisa mexer em código nem servidor.</b><span>O CDM organiza os passos e mantém os dados separados para cada empresa.</span></div></div><div className="setupSteps"><article className={setup.company_ready?'done':''}><b>{setup.company_ready?'✓':'1'}</b><div><h3>Dados da empresa</h3><p>CNPJ, razão social, endereço e inscrição estadual.</p><button className="ghost" onClick={()=>location.hash=''||null}>Confira em “Empresa” no menu</button></div></article><article className={setup.tax_ready?'done':''}><b>{setup.tax_ready?'✓':'2'}</b><div><h3>Tributação</h3><p>CFOP, NCM e regras principais. Configure uma vez com apoio do contador.</p><small>Use Fiscal → Configuração Tributária.</small></div></article><article className={setup.certificate_uploaded?'done':''}><b>{setup.certificate_uploaded?'✓':'3'}</b><div><h3>Certificado digital A1</h3><p>Envie o arquivo .pfx ou .p12 e a senha. O CDM não guarda a senha do certificado.</p><input type="file" accept=".pfx,.p12" onChange={e=>setCert(e.target.files?.[0]||null)}/><input type="password" value={certPass} onChange={e=>setCertPass(e.target.value)} placeholder="Senha do certificado"/><button className="primary" disabled={setupBusy||!cert} onClick={sendCert}>{setupBusy?'Configurando...':'Validar certificado'}</button>{setup.certificate_name&&<small>Certificado configurado: {setup.certificate_name}</small>}</div></article><article className={setup.last_test_status==='ok'?'done':''}><b>{setup.last_test_status==='ok'?'✓':'4'}</b><div><h3>Teste final</h3><p>O CDM confere se tudo está pronto antes de liberar emissão.</p><select value={setup.environment} onChange={e=>{const environment=e.target.value;setSetup(v=>({...v,environment}));saveSetup({environment})}}><option value="homologacao">Homologação (teste, sem valor fiscal)</option><option value="producao">Produção (nota com valor fiscal)</option></select><button className="primary" disabled={setupBusy} onClick={testSetup}>Testar configuração</button>{setup.last_error&&<div className="error">{setup.last_error}</div>}</div></article></div><div className="fiscalSafety"><b>Segurança:</b> tokens e credenciais ficam criptografados; senha do certificado não é armazenada. Antes de usar Produção, faça testes em Homologação.</div></section></>
 ...
 return <><div className="pageTitle fiscalTitle"><div><span>FISCAL</span><h2>Emitir NF-e</h2><p>Ambiente configurado e pronto. O CDM envia e acompanha o retorno do provedor/SEFAZ.</p></div><span className="fiscalEnv">{setup.environment==='producao'?'PRODUÇÃO':'HOMOLOGAÇÃO'}</span></div><section className="panel fiscalPanel"><div className="fiscalTabs">{tabs.map(([k,l])=><button className={section===k?'active':''} key={k} onClick={()=>setSection(k)}>{l}</button>)}</div>
