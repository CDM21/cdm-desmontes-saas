...
def add_customer(data:PartyIn,db:Session=Depends(get_db),user=Depends(current_user)):
...
def add_supplier(data:PartyIn,db:Session=Depends(get_db),user=Depends(current_user)):
...
def part_groups(db:Session=Depends(get_db),user=Depends(current_user)):
    return db.query(PartGroup).filter(PartGroup.company_id==user.company_id).order_by(PartGroup.name).all()
@router.post("/part-groups")
def add_part_group(data:PartGroupIn,db:Session=Depends(get_db),user=Depends(current_user)):
...
def add_location(data:LocationIn,db:Session=Depends(get_db),user=Depends(current_user)):
    row=Location(company_id=user.company_id,**data.model_dump());db.add(row);db.commit();db.refresh(row);return row

@router.get("/tax")
def tax(db:Session=Depends(get_db),user=Depends(current_user)):
    ...
    return row
@router.put("/tax")
def update_tax(data:TaxIn,db:Session=Depends(get_db),user=Depends(current_user)):
    row=db.query(TaxConfig).filter(TaxConfig.company_id==user.company_id).first()
...
@router.post("/users")
def add_user(data:UserIn,db:Session=Depends(get_db),user=Depends(current_user)):
