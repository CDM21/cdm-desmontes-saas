...
@router.get("")
def list_vehicles(db:Session=Depends(get_db), user=Depends(current_user)):
    return db.query(Vehicle).filter(Vehicle.company_id==user.company_id).order_by(Vehicle.id.desc()).all()

@router.post("")
def create_vehicle(data:VehicleIn, db:Session=Depends(get_db), user=Depends(current_user)):
    v=Vehicle(company_id=user.company_id,**data.model_dump()); db.add(v); db.commit(); db.refresh(v); return v

@router.get("/{vehicle_id}")
def get_vehicle(vehicle_id:int,db:Session=Depends(get_db),user=Depends(current_user)):
...
def start_dismantling(vehicle_id:int,db:Session=Depends(get_db),user=Depends(current_user)):
    v=db.query(Vehicle).filter(Vehicle.id==vehicle_id,Vehicle.company_id==user.company_id).first()
    if not v: raise HTTPException(404,"Veículo não encontrado")
